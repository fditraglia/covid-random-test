...
[^5]: The calculations in this paragraph are based on a power calculation for a test of the null hypothesis $H_0\colon p \leq 0.0025$ against the one-sided alternative $H_1\colon p > 0.0025$ with level $\alpha = 0.1$ using the score test statistic $T_n = (\widehat{p} - p_0)/ SE(p_0)$ where $SE(p_0)$ is the theoretical standard error under the null with a finite-population correction, i.e.\ $SE(p_0) = \sqrt{\kappa p_0 (1 - p_0)/n}$ where $\kappa = (N - n)/(N - 1)$ and $N$ is the population size.
