...
Given what we know about the spread of coronavirus, it is likely that infected households will begin to *cluster* within halls of residence and possibly colleges as the term progresses. Suppose that we test a simple random sample of 300 households during week 1 of MT. If there are roughly 35 infected households at the start of term, then we have a `r 100 - round(100 * get_prob_E_range(lower = 0, upper = 0, n = 300, C = 35))`\% of detecting *at least one of them* in this first round of testing. Let $P$ be the households that test positive. In week two, rather than using all 300 of our weekly tests for a second round of random sampling, we could divert some of them to systematically testing the *neighbors* of $P$: say the households on adjacent floors of the same residence hall. If any of these neighboring households test positive, we could then go on to test *their* neighbors, and so on. This idea is called *adaptive cluster sampling*.[^adapt]
